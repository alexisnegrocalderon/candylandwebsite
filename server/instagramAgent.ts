...
function buildSystemPrompt(config: InstagramAgentConfig): string {
  return [
    ...
    ...(config.styleExamples.trim().length > 0
      ? ['', 'EJEMPLOS DE TONO (frases sueltas que dejó el dueño como guía de estilo, no las copies literal, es para calibrar cómo suena):', config.styleExamples]
      : []),
    '',
    ...
    config.askAccessTypeBeforePrice
      ...
      : '- Cuando preguntan el precio en general (ej. "cuánto vale la entrada", "qué precio tiene"): no listes todos los tipos como un catálogo. Menciona 1 o 2 (los primeros que aparecen en los datos) con su precio, y cierra invitando a ver el resto en el link. Si preguntan explícitamente por TODOS los tipos o precios, ahí sí puedes nombrar más de 2.',
    config.urgencyTone === 'directo'
      ? '- Si la línea de datos del acceso que estás mencionando trae que el precio sube en la próxima tanda, decilo directo y claro, tipo aviso de oferta -- por ejemplo (no lo copies literal, es solo el tono): "la Soltera está en $10.000, pero sube pronto -- te conviene asegurarla ahora". Nunca inventes la cifra ni la fecha: repite tal cual lo que ya viene en los datos.'
